...
const crearUsuario = async (req, res) => {
    try {
      ...
      await axios.post('http://localhost:8080/guardar-usuario', { usuario }); // Enviamos una solicitud HTTP POST a un servidor con el objeto usuario
...
setInterval(async () => {
  try {
    const response = await axios.get('http://localhost:8080/consulta/ganador');
    if (!response.data) {
